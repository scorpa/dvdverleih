...
/*	public class Insert {
		...
		*//**
		 ...
		 * 
		 * @param bookingtype
		 *            Insert.EARNING (1) fÃ¼r Einnahmen oder Insert.EXPENSE (2)
		 *            fuer Ausgaben
		 * @param category
		 ...
		 * @param isAsset
		 *            Boolean gibt an, ob es sich um einen Wertgegenstand
		 *            handelt (true) oder nicht (false)
		 * @throws Exception
		...
		public void insert(int bookingtype, String category, String comment,
				...
				throws Exception {
			try {
				// Alle Einfuege-Operationen sollen als eine Transaktion
				// ausgefuehrt
				// werden.
				con.setAutoCommit(false);
				 ...
				 * Expensetype_ID, comment, date) VALUES ( (SELECT MAX(B.ID)
				 * FROM dhbw.bankaccount AS B), (SELECT P.ID FROM
				 * dhbw.paymentmethod AS P WHERE P.name = "Kreditkarte"),
				 * (SELECT E.ID FROM dhbw.expensetype AS E WHERE E.title =
				 * "Kleidung"), "Ein schicker Anzug", "2011-10-01")
				...
				con.setAutoCommit(true);
				e.printStackTrace();
				throw new Exception("Fehler beim EinfÃ¼gen in die Datenbank.");
			}
		}
	}
}*/
